...
- Simulator run `36152347324` built the grouped ScrollView revision but still timed out on a Settings child navigation query; its captured screen stayed on Settings and the process sample included Settings and Local data body updates. Settings rows now present their existing detail views in an independent full-screen NavigationStack with a Settings return button. Focused simulator run `36154745842` at `0941bb4` passed `testMoreAndSettingsDestinationsRemainNavigable`: one test, zero failures, including all four Your ledger destinations and return navigation. Physical-device verification remains pending.
...
- XCTest activity extraction identified Local data as the exact tap. Manual run `36081242490` captured main-thread work in `SettingsView.body`, `LocalDataSettingsView.body`, SwiftData fetches, and SwiftUI list updates during the stall. Value-based NavigationLinks made the smoke fail earlier at Settings in `36135497205`, so they were reverted. The separate full-screen presentation now passes the simulator navigation test; physical-device verification remains pending.
- An earlier accessibility-containment follow-up was locally contract-checked but failed on the owner's iPhone. It is retained as a separate accessibility improvement, not credited as the navigation fix.
