...
> Status: Milestones 0–2 and Milestone 4’s backend transport pass their local Linux gates. Milestone 5 native source now includes offline transfer/refund/tag flows, combined discovery, collaborator-role caching, viewer-safe mutations, owner/admin tracker presentation and ordering, explicit sync/no-access states, and an authored 50,000-record regression check. Native source passes Linux static contracts, but Swift compilation/runtime still requires hosted macOS. Milestone 6 Shortcut automation is next. Binary receipt transfer remains Milestone 9. Docker/hosted-macOS workflows have not run, nothing has been deployed, and no IPA has been signed or tested on a physical iPhone.
