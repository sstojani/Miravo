...
> Status: Milestones 0–2 and Milestone 4’s backend transport pass their local Linux gates. Milestones 5–8 are source-complete at the available tier: the native app now includes offline participants, expense splits, balances, settlements, connected invite/member controls, and guarded guest merge alongside budgets, recurrence, and installments. The complete backend gate passes 85 tests with 81.20% branch-aware coverage; Linux localization/privacy/contracts and targeted Swift syntax parsing pass, but Swift compilation and native tests still require macOS. Notification/Wallet/two-device execution, Docker, and hosted CI remain unavailable here. Nothing has been deployed, and no IPA has been signed or device-tested.
