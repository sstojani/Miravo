...
> Status: Milestones 0–2 and Milestone 4’s backend transport pass their local Linux gates. Native local-first/sync source and tests—including outbox replay, bounded pull/bootstrap staging, conflicts, diagnostics, access revocation, an attachment-transfer queue scaffold, WebSocket/polling invalidation, connectivity return, and best-effort background refresh—are implemented and pass Linux static checks, but Swift compilation still requires hosted macOS. Binary receipt transfer remains Milestone 9. Docker/hosted-macOS workflows have not run, nothing has been deployed, and no IPA has been signed or tested on a physical iPhone.
