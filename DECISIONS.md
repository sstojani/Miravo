...
## D-030 — Explicit, generic, bounded local reminders

- **Decision:** Keep recurring reminders disabled until the user explicitly enables them and grants ordinary local-notification permission. Schedule only the earliest 50 active future rules for the current server/user scope, with due-time/one-day/three-day/one-week lead choices. Store the preference under a hashed scope key, derive opaque notification identifiers from the scope and rule UUID, and use one generic title/body that contains no rule name, amount, currency, merchant, note, or provider. Reconcile after local rule changes and successful sync; remove the signed-out scope's requests. Local notification delivery never materializes an occurrence or changes financial state.
- **Why:** Lock-screen previews and pending-notification identifiers are observable surfaces, while iOS imposes finite notification capacity and can deny or revoke permission at any time. A user-visible opt-in plus generic content minimizes disclosure, and a deterministic bound leaves capacity for later installment/budget reminders without making planning correctness depend on the OS scheduler.
- **Consequence:** The user sees permission, scheduled-count, denial, and Settings-recovery states in Plans. Reminders for later rules are added during a subsequent reconciliation as nearer requests disappear or rules advance. App locking leaves requests intact, while sign-out removes them for local privacy; signing back in reschedules from the still-scoped preference. Planner/controller tests and a Linux source contract cover the design, but actual authorization prompts, notification delivery, and third-party-signer behavior remain device checks.
