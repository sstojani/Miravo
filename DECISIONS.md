...
The first targeted accessibility containment change in `ca09515` did not resolve the freeze on the owner's iPhone. All four Your ledger destinations still fail, so the shared parent Settings Form is the next isolation target. Use a grouped ScrollView for that parent while keeping the same destination views and controls; require a focused native navigation pass before treating the change as resolved.
...
## D-040 — Bundled server URL and explicit Shortcut capture alerts

- **Decision:** Let the iOS build carry a non-secret `MIRAVO_DEFAULT_SERVER_URL` value in `MiravoDefaultServerURL`. When present, login uses that URL automatically and asks only for email/password; manual URL entry remains a fallback for unconfigured builds and debugging. Shortcut-created posted expenses may show an explicit local Miravo notification with amount and merchant after the app learns about the transaction through sync.
- **Why:** The owner wants cloud sync to feel automatic rather than asking for a server URL on every install, while server origin still must be centralized and release-configured instead of guessed. Shortcut capture is a user-initiated event where an amount/place confirmation is useful; generic planning reminders still avoid private preview content.
- **Consequence:** A release/IPA intended for real use must set `MIRAVO_DEFAULT_SERVER_URL` to the actual HTTPS API origin before signing. Without APNs, App Intents, or an extension, Shortcut expense notifications can only be delivered by Miravo after foreground/background sync runs and iOS notification permission is already authorized; the Shortcut itself remains responsible for immediate Shortcut-app feedback if Miravo has not run yet.

## D-041 — Reinstall restore takes precedence over local starter data

- **Decision:** After an authenticated first bootstrap, Miravo creates the local Everyday/Cash/General starter set only if the server sync succeeds, bootstrap is no longer required, and the restored scope still has no available tracker. Overview and Plans prefer a tracker that already has transactions, budgets, recurring rules, or installment plans before falling back to the first available tracker. The onboarding flow uses swipe-first pages and places email/password sign-in on the final page; continuing as a guest requires an explicit local-only data-loss warning.
- **Why:** A real reinstall showed that transactions could be visible while Overview and Plans selected an empty starter tracker, making restored budgets/plans look missing. Guest mode is useful, but it must not feel equivalent to a cloud-backed login.
- **Consequence:** Existing empty starter trackers are not destructively removed, but restored server data becomes the primary visible state. Brand-new cloud accounts still get starter data only after a confirmed empty bootstrap. SwiftUI runtime layout, animation smoothness, and physical reinstall recovery remain GitHub macOS/device checks.

## D-042 — Passive screens stay quiet and notification permission is system-led

- **Decision:** Plans reselects a restored tracker while plan rows are still arriving unless the user has manually chosen another tracker. Miravo asks iOS for notification permission from the main app flow instead of exposing duplicate notification toggles in Plans and Settings; after denial, recovery is handled through the iPhone Settings app. Passive Collaboration screen loading uses silent synchronization so a transient server failure appears in the Collaboration notice area instead of a global modal. Shortcut tracker defaults use a narrow editor rather than navigating into the full Local Data screen.
- **Why:** Device testing showed plans existed locally but an empty tracker could remain selected, notification controls were duplicated and confusing, Collaboration surfaced an alarming popup on entry, and the deep Shortcut defaults route could freeze the app.
- **Consequence:** Restored budgets/recurring/installments become visible as soon as their rows arrive. Notification scheduling still depends on iOS authorization and remains non-authoritative for financial correctness. Passive network failures no longer interrupt navigation, while explicit sync actions can still surface errors. The revised SwiftUI behavior requires macOS CI and owner device verification.

## D-043 — Guest sign-in adopts real local work, not disposable defaults

- **Decision:** Signing in from a local guest profile now switches the active scope to the authenticated server/user identity before synchronization. The local store inspects the previous guest scope once: the exact untouched starter profile is deleted locally, while any profile containing real local records is moved into the authenticated scope and pending outbox operations are resequenced after existing authenticated work.
- **Why:** Device testing showed repeated delete/reinstall/guest/sign-in cycles could create duplicate Everyday trackers, Cash accounts, categories, and owner rows. Guest mode still needs to preserve actual offline entries, but an empty starter scaffold should not become cloud data.
- **Consequence:** Brand-new guest users who sign in without entering data restore the server account instead of uploading a duplicate starter tracker. Guest users who entered expenses, budgets, plans, splits, attachments, or taxonomy changes keep those records and they sync under the signed-in account. Existing duplicate cloud rows are not destructively cleaned up by this client-side fix; cleanup needs a separate explicit data-repair operation.

## D-044 — Existing duplicate starter ledgers require explicit dry-run repair

- **Decision:** Do not hide or automatically merge duplicated server trackers in the app. Provide an operator-run backend command that reports duplicate active owner trackers for one account and deletes only empty starter duplicates after `--confirm`; any duplicate containing transactions, budgets, recurring rules, installment plans, settlements, or attachments is kept for manual review.
- **Why:** The duplicate rows may already be synchronized cloud data, and the app cannot reliably know which ledger the owner wants to keep once multiple tracker scopes exist. Automatically deleting from the client could remove real financial history or hide a server-side data problem.
- **Consequence:** The prior guest adoption fix prevents new empty guest starter uploads, while existing cloud duplicates can be repaired deliberately from the server. The cleanup still emits normal tracker tombstones through the sync log so the phone can remove those duplicate scopes after the next pull.
