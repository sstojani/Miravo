...
| Plans | Native tracker/budget/recurrence pickers and forms; text plus symbols for progress, due/overdue, subscription, occurrence, archive, and sync states; reminder permission/count/denial are textual and the Settings route is labeled; icon-only add/options controls have explicit labels; viewer controls are omitted while records remain readable | Large monetary/cost values, VoiceOver card order and context-menu discoverability, notification-permission prompt/focus, sheet focus, date/time controls, and Albanian accessibility-size layout |
...
2. Test the largest accessibility Dynamic Type sizes in English and Albanian on Quick Add, transaction list/detail, Plans, Local Data, conflict review, and Wallet Shortcut management.
