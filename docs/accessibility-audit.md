...
| Plans | Native tracker/budget/recurrence pickers and forms; text plus symbols for progress, due/overdue, subscription, occurrence, archive, and sync states; icon-only add/options controls have explicit labels; viewer controls are omitted while records remain readable | Large monetary/cost values, VoiceOver card order and context-menu discoverability, sheet focus, date/time controls, and Albanian accessibility-size layout |
