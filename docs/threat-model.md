...
| Lock-screen notification disclosure | Recurring/installment reminders are off by default, require explicit permission, share one bounded queue, use generic title/body copy, and carry only opaque hashed request identifiers; sign-out removes the active scope's pending requests | The existence and timing of a reminder can still reveal that the app is used; verify previews and signer behavior on the physical device |
...
| Leaked tracker invite | Independent email-bound one-time HMAC token, short configurable expiry, immediate revoke, ordinary authenticated acceptance, app memory-only redacted wrapper, expiring local-only copy | Intended email account compromise can consume it; screenshots/keyboards/clipboard observers remain risks, so send privately and revoke misdirected codes |
