...
Miravo is the final product name; Project Ledger remains the internal code/module/service namespace. Milestones 0–2 and Milestones 4–8 are complete at the available local/source tier. Milestone 9's private-receipt vertical slice now spans transaction-scoped server metadata, isolated content/quarantine storage, authorized download, tombstone sync, a protected native upload queue, camera/photo/file/PDF preparation, on-device OCR review, offline preview, and authenticated cross-device retrieval. The analytics vertical slice now adds deterministic backend summary reporting and matching offline native Insights source. Exports are the next active slice. Docker/PostgreSQL/Redis integration, macOS compilation/native test execution, real camera/OCR and multi-device execution, hosted CI, and remote publishing remain environmental checks. No production system has been contacted.
...
- `make check` passes: 93 tests, 82.06% coverage, Ruff, strict mypy over 107 source files, Django checks, and OpenAPI validation/freshness.
