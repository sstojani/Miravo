...
Miravo is the final product name; Project Ledger remains the internal code/module/service namespace. Milestones 0–2 and Milestones 4–8 are complete at the available local/source tier. Milestone 9's private-receipt vertical slice now spans transaction-scoped server metadata, isolated content/quarantine storage, authorized download, tombstone sync, a protected native upload queue, camera/photo/file/PDF preparation, on-device OCR review, offline preview, and authenticated cross-device retrieval. The analytics vertical slice adds deterministic backend summary reporting and matching offline native Insights source. The backend export slice adds CSV, text-PDF, and full JSON jobs with private storage, requester-only expiring downloads, checksums, and audit. Native export UI/download management is the next active slice. Docker/PostgreSQL/Redis integration, macOS compilation/native test execution, real camera/OCR and multi-device execution, hosted CI, and final device validation remain environmental checks. No production system has been contacted.
...
- English/Albanian implemented-screen coverage (725 literal keys), release/debug ATS separation, privacy-manifest disclosure, and YAML/plist/JSON resource structure pass locally. The source accessibility audit and outstanding runtime checks are in `docs/accessibility-audit.md`.
