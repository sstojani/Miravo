...
Milestones 0–2 and Milestones 4–6 are complete at the available local/source tier. Milestone 7 includes offline-first budgets; recurrence/subscriptions with a native scoped cache and privacy-safe local reminders; and a locally passing authoritative installment backend with deterministic component schedules, revisions, linked ledger payments, extra payments, skip/reschedule, payoff, explicit overpayment, and REST/sync transport. Native installment persistence and Plans UI are next. Binary receipt transport remains Milestone 9. Docker/PostgreSQL/Redis integration and hosted CI remain environmental checks. No production system has been contacted.
...
- `make check` passes: 80 tests, 81.74% branch-aware coverage, Ruff, strict mypy over 93 source files, Django checks, and OpenAPI validation/freshness.
...
- Native installment persistence/outbox/calculator/UI is not yet implemented; its backend wire contract is current and locally verified.
