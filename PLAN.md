...
The bottom navigation is being restyled as a floating icon pill with a More hub that exposes account sign-in/disconnect plus Settings/Insights entry points. Settings now includes a persistent Appearance preference for System, Light, or Dark mode. Quick Add gives its save action explicit clearance from the floating pill, hides the pill while the keyboard is visible, and limits the undo confirmation to a three-second transient row. Transaction-row destructive swipe actions are explicitly tinted with the Miravo destructive color instead of inheriting the global accent. Public self-registration remains intentionally absent; additional users are still created by the server-side `create_app_user` operator command until a reviewed invitation/signup flow is designed.
