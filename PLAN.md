...
The bottom navigation is being restyled as a solid floating icon pill with a transparent surrounding bottom area and a More hub that exposes account sign-in/disconnect plus Settings/Insights entry points. Settings now includes a persistent Appearance preference for System, Light, or Dark mode. Quick Add hides its bottom action until a draft exists, hides the pill while the keyboard is visible, uses a compact floating Save capsule, and replaces Save with an Undo capsule for three seconds after saving before hiding again. Transaction-row destructive swipe actions are explicitly tinted with the Miravo destructive color instead of inheriting the global accent. Successful server authentication now also completes the onboarding gate so a signed-in relaunch does not show the first-open slides again, and first launch after reinstall attempts same-device Keychain recovery before falling back to onboarding. Public self-registration remains intentionally absent; additional users are still created by the server-side `create_app_user` operator command until a reviewed invitation/signup flow is designed.
...
Current focus: **Milestone 9/10 — native restore/login polish and hosted/device verification**. Miravo is the final product name and `https://github.com/sstojani/Miravo.git` is populated. Local `main` has been fast-forwarded to remote `main` at `9721156`, including the iOS experience/notification, Quick Add focus, sync dependency-order, and reinstall restore fixes. The app now bundles the provided HTTPS server origin in `MIRAVO_DEFAULT_SERVER_URL` so normal login can be email/password-only; server override lives in Settings for local profiles and remains a fallback/debug path. Native first-bootstrap provisioning is guarded so a nonempty server restore is not masked by an empty starter tracker, Plans reselects the restored tracker when budget/recurring/installment rows arrive after the screen opens, and guest-to-login adoption now switches to the authenticated server/user scope before sync. An untouched guest starter profile is discarded instead of uploaded as duplicate cloud data; guest profiles containing real local records are moved into the authenticated scope with pending outbox order preserved. Existing duplicated cloud starter ledgers are handled by an explicit dry-run-first backend cleanup command that deletes only empty duplicates after confirmation. Onboarding is swipe-first with a fourth sign-in slide and an explicit guest-mode local-only warning; the sign-in form now uses shared icon-led material auth controls instead of the earlier boxed field stack. Shortcut-created expenses now produce explicit local Miravo notifications after successful sync when iOS notification permission is authorized, and Miravo requests the one-time iOS notification prompt automatically instead of showing duplicate in-app notification toggles. Collaboration and Shortcut setup no longer use passive screen entry to surface global server-unreachable alerts or deep Local Data navigation. Native export listing now decodes the server's paginated job response and downloads CSV/PDF/JSON files with a broad binary Accept header before handing verified writable documents to iOS. Xcode execution, physical Shortcut delivery, Docker/Linux file-mode behavior, unsigned IPA, and device validation remain external until Actions and user-side signing/device checks pass.
