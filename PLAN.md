...
Current focus: **Milestone 9/10 — native restore/login polish and hosted/device verification**. Miravo is the final product name and `https://github.com/sstojani/Miravo.git` is populated. Local `main` has been fast-forwarded to remote `main` at `9721156`, including the iOS experience/notification, Quick Add focus, sync dependency-order, and reinstall restore fixes. The app now bundles the provided HTTPS server origin in `MIRAVO_DEFAULT_SERVER_URL` so normal login can be email/password-only; server override lives in Settings for local profiles and remains a fallback/debug path. Native first-bootstrap provisioning is guarded so a nonempty server restore is not masked by an empty starter tracker, and Plans now reselects the restored tracker when budget/recurring/installment rows arrive after the screen opens. Onboarding is swipe-first with a fourth sign-in slide and an explicit guest-mode local-only warning; the sign-in form uses lightweight material fields instead of a heavy settings-style card. Shortcut-created expenses now produce explicit local Miravo notifications after successful sync when iOS notification permission is authorized, and Miravo requests the one-time iOS notification prompt automatically instead of showing duplicate in-app notification toggles. Collaboration and Shortcut setup no longer use passive screen entry to surface global server-unreachable alerts or deep Local Data navigation. Xcode execution, physical Shortcut delivery, Docker/Linux file-mode behavior, unsigned IPA, and device validation remain external until Actions and user-side signing/device checks pass.
