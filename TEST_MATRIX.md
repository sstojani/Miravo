...
| Shortcut app/setup | Create/list/revoke token without persisting the raw value; choose defaults; reproducible online and bounded offline-queue construction | Native tests pending; checked-in single/batch JSON samples validate syntactically | Current-iPhone Wallet trigger, duplicate notification, queue rewrite/flush | planned |
...
| Localization | Every implemented user string exists in English and Albanian | `check-localizations.sh` and 294-key coverage/format check | Native-speaker review | passing-local |
