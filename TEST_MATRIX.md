...
| Transaction discovery | Debounced local search over related names/amount and combined tracker/account/category/type/source/status/sync/currency/date facets; currencies remain separate in totals | `TransactionListQueryTests.swift` (authored) | VoiceOver/filter reset and 50k-record responsiveness | implemented |
| Sync server | Ordered per-operation push, replay/fingerprint safety, signed user-bound cursor, paging/tombstone/paginated-bootstrap/ack, stable bootstrap target token, retention, revocation filtering | `test_sync.py` (10 tests) | Docker/PostgreSQL concurrency smoke test | passing-local |
...
| Localization | Every implemented user string exists in English and Albanian | `check-localizations.sh` and 249-key coverage/format check | Native-speaker review | passing-local |
