...
EXPORT_EXPIRY_HOURS = env_int("PROJECT_LEDGER_EXPORT_EXPIRY_HOURS", 24)
ATTACHMENT_MAX_BYTES = env_int("PROJECT_LEDGER_ATTACHMENT_MAX_BYTES", 12 * 1024 * 1024)
ATTACHMENT_UPLOAD_CHUNK_BYTES = 64 * 1024
ATTACHMENT_SCANNER = os.getenv("PROJECT_LEDGER_ATTACHMENT_SCANNER", "").strip()
DATA_UPLOAD_MAX_MEMORY_SIZE = ATTACHMENT_MAX_BYTES
...
SYNC_RETENTION_DAYS = env_int("PROJECT_LEDGER_SYNC_RETENTION_DAYS", 90)
SYNC_OPERATION_RECEIPT_DAYS = env_int("PROJECT_LEDGER_SYNC_OPERATION_RECEIPT_DAYS", 120)
...
BUDGET_MAX_ROLLOVER_PERIODS = env_int("PROJECT_LEDGER_BUDGET_MAX_ROLLOVER_PERIODS", 600)
RECURRING_MAX_OCCURRENCES_PER_RULE_RUN = env_int(
    "PROJECT_LEDGER_RECURRING_MAX_OCCURRENCES_PER_RULE_RUN", 100
)
RECURRING_MAX_RULES_PER_RUN = env_int("PROJECT_LEDGER_RECURRING_MAX_RULES_PER_RUN", 500)
...
MAXIMUM_RECURRING_OCCURRENCES_PER_RULE_RUN = 1_000
MAXIMUM_RECURRING_RULES_PER_RUN = 10_000
MAXIMUM_ATTACHMENT_BYTES = 64 * 1024 * 1024
...
if SYNC_OPERATION_RECEIPT_DAYS < SYNC_RETENTION_DAYS:
    raise ImproperlyConfigured(
        "PROJECT_LEDGER_SYNC_OPERATION_RECEIPT_DAYS must cover the sync retention window"
    )
if not 1 <= SYNC_MAX_PUSH_OPERATIONS <= MAXIMUM_SYNC_PUSH_OPERATIONS:
...
if not (1 <= RECURRING_MAX_OCCURRENCES_PER_RULE_RUN <= MAXIMUM_RECURRING_OCCURRENCES_PER_RULE_RUN):
    raise ImproperlyConfigured(
        "PROJECT_LEDGER_RECURRING_MAX_OCCURRENCES_PER_RULE_RUN must be between 1 and 1000"
    )
if not 1 <= RECURRING_MAX_RULES_PER_RUN <= MAXIMUM_RECURRING_RULES_PER_RUN:
    raise ImproperlyConfigured(
        "PROJECT_LEDGER_RECURRING_MAX_RULES_PER_RUN must be between 1 and 10000"
    )
if not 1 <= ATTACHMENT_MAX_BYTES <= MAXIMUM_ATTACHMENT_BYTES:
    raise ImproperlyConfigured("PROJECT_LEDGER_ATTACHMENT_MAX_BYTES must be between 1 and 67108864")
...
CELERY_BEAT_SCHEDULE = {
    ...
    "materialize-recurring-rules": {
        "task": "apps.planning.tasks.materialize_recurring_rules",
        "schedule": 300.0,
    },
}
